export default {
  lowerBody: [
    {name: 'Quadriceps muscles group', left: 58.70967741935483, top: 57.078651685393254, checked: false},
    {name: 'Hamstrings muscles group', left: 50, top: 57.752808988764045, checked: false},
    {name: 'Gluteus muscles group', left: 48.225806451612904, top: 28.98876404494382, checked: false},
    {name: 'Soleus, Gastrocnemius', left: 81.42857142857143, top: 63.75404530744336, checked: false},
  ],
  upperBodyFront: [
    {
      name: 'Pectoralis major clavicular head',
      left: 36.868686868686865,
      top: 22.15568862275449,
      checked: true
    },
    {name: 'Brachioradialis', left: 90, top: 69.49152542372882, checked: false},
    {name: 'Flexors muscles group', left: 17.42857142857143, top: 85.76271186440678, checked: false},
    {name: 'Extensors muscles group', left: 90.28571428571428, top: 86.10169491525423, checked: false},
    {name: 'Biceps brachii long head', left: 81.28571428571428, top: 56.949152542372886, checked: false},
    {name: 'Biceps brachii short head', left: 75.14285714285714, top: 43.050847457627114, checked: false},
    {name: 'Trapezius superior fibers', left: 60.57142857142858, top: 5.101694915254238, checked: false},
    {name: 'Pectoralis major sternal head', left: 35.18518518518518, top: 39.12175648702595, checked: true},
    {name: 'Deltoid anterior head', left: 76.43097643097643, top: 19.36127744510978, checked: false},
    {name: 'Rectus abdominis', left: 48.505050505050505, top: 67.46506986027944, checked: false},
    {name: 'Abdominal obliques', left: 30.285714285714285, top: 63.33898305084746, checked: false},
  ],
  upperBodyBack: [
    {name: 'Trapezius inferior fibers and rhomboid', left: 53.80952380952381, top: 21.57676348547718, checked: false},
    {name: 'Triceps brachii long head', left: 27.142857142857142, top: 55.601659751037346, checked: false},
    {name: 'Triceps brachii lateral head', left: 10.714285714285714, top: 65.14522821576763, checked: false},
    {name: 'Teres minor, Infraspinatus, Supraspinatus, Subscapularis', left: 37.38095238095238, top: 29.87551867219917, checked: false},
    {name: 'Latissimus dorsi', left: 39.523809523809526, top: 64.31535269709543, checked: false},
    {name: 'Deltoid lateral head', left: 10.952380952380953, top: 19.08713692946058, checked: false},
    {name: 'Deltoid posterior head', left: 25.476190476190474, top: 14.107883817427386, checked: false},
  ]
}